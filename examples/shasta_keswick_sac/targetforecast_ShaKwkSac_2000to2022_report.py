...
#%%
btime_all = time.perf_counter()
for yr in range(2000, 2023): # 2023): #[2017]: #, 2015, 2016]:
    
    btime_this = time.perf_counter()
    # reinitialize coupled model
    if yr==2021 or yr==2020: #2010:
        couple_config_fp = os.path.join(curdir, r'coupled_shakwkriv_2021.yaml')
    else:
    ...
    for d in cpl.SimDates: #[0:154]: #123 = May 2]:
        ...
        if d.date().month==1 and d.date().day==1:
            gate_mode='full' # override the gate mode on the first day of year and force a full search 
        ...
        if gate_mode=='full':
            ...
            if d.date() == dt.date(2000,1,1):
                prev_gates = {3:0,2:5,1:0,0:0}
            elif d.date().year>=2001 and d.date().month==1 and d.date().day==1:
                prev_gates = final_gates_
            else:
...
#%  
    ...
    if save_plots:
        ...
        ff =uppsac.plot_sim_obs(cpl.Temperature_Target.Target_Location,
                                plot_units='degC', year=yr,
                            temp_targ = cpl, add_tol=True, plot_off=True)
        ccrofp = os.path.join(figdir, f'{todaystr}DRAFT_CCR_RivTempFcstTarg_{yr}_degC.png')
        ...
        for n in uppsac.Nodes:
            if n.lower() != 'ccr':
                if uppsac.Nodes[n].UpNode>=0:
                    ...
                    f.tight_layout()
                    ofn = f'{todaystr}DRAFT_{n.upper()}_RivTempFcstTarg_{yr}_degC.png'
    ...
    if save_plots:
        ...
        shrelfp = os.path.join(figdir, f'{todaystr}DRAFT_ShastaRelease_FcstTarg_{yr}.png')
        ...
        shproffp = os.path.join(figdir, f'{todaystr}DRAFT_ShastaProfiles_FcstTarg_{yr}.png')
        plt.savefig(shproffp, dpi=600)
        plt.close()
    
        f = mkp.plotStorageEvapCompare(sha, viewSave='view')
        evapstofp = os.path.join(figdir, f'{todaystr}DRAFT_ShastaEvapStorage_FcstTarg_{yr}.png')
